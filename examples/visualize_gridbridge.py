...
# Try to import pypsa for network visualization
try:
    import pypsa
    HAS_PYPSA = True
except ImportError:
...
def create_architecture_diagram(output_path: Path):
    ...
    ax.set_ylim(0, 100)
    ax.axis('off')
    ax.set_title('GridBridge UK - System Architecture', fontsize=18, fontweight='bold', pad=20)
    ...
    # Title box
    ax.text(50, 97, 'GRIDBRIDGE UK PLATFORM', ha='center', va='center',
            fontsize=16, fontweight='bold', color='#2c3e50')
    ax.text(50, 94, 'AI-Powered Grid Connection Acceleration', ha='center', va='center',
    ...
    ax.text(2, y_data + 4, 'DATA\nSOURCES', ha='center', va='center', fontsize=9,
            fontweight='bold', color='#7f8c8d')

    # Layer 2: Ingest Layer
    y_ingest = 68
    draw_box(15, y_ingest, 70, 8, 'DATA INGESTION LAYER', colors['ingest'],
    ...
    y_core = 50
    draw_box(10, y_core, 24, 12, 'GRID MODEL\nLAYER', colors['model'],
             'AC Power Flow\nN-1/N-2 Analysis')
    draw_box(38, y_core, 24, 12, 'AI/ML\nENGINE', colors['ai'],
             'Forecasting\nScenario Gen')
    draw_box(66, y_core, 24, 12, 'OPTIMISATION\nENGINE', colors['optimize'],
             'MILP/Convex\nPareto Ranking')
    ...
    y_reg = 36
    draw_box(15, y_reg, 70, 8, 'REGULATORY CONSTRAINTS LAYER', colors['regulatory'],
    ...
    y_ui = 18
    draw_box(8, y_ui, 25, 12, 'DEVELOPER\nPORTAL', colors['ui'],
             'Site Search\nConnection Options')
    draw_box(37, y_ui, 26, 12, 'NETWORK\nOPERATOR PORTAL', colors['ui'],
             'Queue Analytics\nFlexibility Mgmt')
    draw_box(67, y_ui, 25, 12, 'REGULATOR\nDASHBOARD', colors['ui'],
             'Capacity Metrics\nAudit Trail')
    ...
    # Footer
    ax.text(50, 2, 'GridBridge UK © 2026 | Accelerating Grid Connections for AI Data Centres',
            ha='center', va='center', fontsize=9, color='#95a5a6')

    plt.tight_layout()
    plt.savefig(output_path / 'architecture_diagram.png', dpi=150, bbox_inches='tight',
...
def create_data_flow_diagram(output_path: Path):
    ...
    ax.set_ylim(0, 100)
    ax.axis('off')
    ax.set_title('GridBridge UK - Data Flow & API Integration', fontsize=16, fontweight='bold', pad=20)

    # API Sources (left side)
    apis = [
        ('Carbon Intensity API', 'api.carbonintensity.org.uk', '30min', 'FREE', '#27ae60'),
        ('Elexon BMRS (Legacy)', 'api.bmreports.com', '5min-30min', 'API KEY', '#3498db'),
        ('Elexon Insights', 'data.elexon.co.uk', '30min', 'FREE*', '#3498db'),
        ('ESO Data Portal', 'data.nationalgrideso.com', 'varies', 'FREE', '#9b59b6'),
        ('Met Office DataPoint', 'datapoint.metoffice.gov.uk', '1hr', 'API KEY', '#e67e22'),
    ...
    for i, (name, url, res, auth, color) in enumerate(apis):
        y = y_start - i * y_step
        # API box
        box = FancyBboxPatch((2, y-4), 35, 8, boxstyle="round,pad=0.02",
                             facecolor=color, edgecolor='white', linewidth=2, alpha=0.85)
        ax.add_patch(box)
        ax.text(19.5, y + 1, name, ha='center', va='center', fontsize=10, fontweight='bold', color='white')
        ax.text(19.5, y - 2, url, ha='center', va='center', fontsize=7, color='white', alpha=0.9)

        # Resolution & Auth badges
        ax.text(39, y + 1.5, res, ha='left', va='center', fontsize=8,
        ...
        # Arrow to processing
        ax.annotate('', xy=(52, 50), xytext=(37, y),
                    arrowprops=dict(arrowstyle='->', color='#bdc3c7', lw=1,
                                    connectionstyle='arc3,rad=0.1'))

    # Central Processing Box
    proc_box = FancyBboxPatch((52, 35), 20, 30, boxstyle="round,pad=0.02",
                              facecolor='#2c3e50', edgecolor='#34495e', linewidth=3)
    ax.add_patch(proc_box)
    ax.text(62, 58, 'INGEST', ha='center', va='center', fontsize=11, fontweight='bold', color='white')
    ax.text(62, 54, '& CANONICALISE', ha='center', va='center', fontsize=9, color='#bdc3c7')

    # Processing steps
    steps = ['Fetch API', 'Parse Response', 'Validate Schema', 'Align Time', 'Quality Check', 'Merge Sources']
    for i, step in enumerate(steps):
        ax.text(62, 49 - i*3, f'→ {step}', ha='center', va='center', fontsize=8, color='#ecf0f1')
    ...
    for i, (name, desc, color) in enumerate(outputs):
        y = y_out - i * 18
        box = FancyBboxPatch((77, y-5), 20, 10, boxstyle="round,pad=0.02",
                             facecolor=color, edgecolor='white', linewidth=2, alpha=0.9)
        ax.add_patch(box)
        ax.text(87, y + 2, name, ha='center', va='center', fontsize=10, fontweight='bold', color='white')
        ax.text(87, y - 2, desc, ha='center', va='center', fontsize=7, color='white', alpha=0.9)

        # Arrow from processing
        ax.annotate('', xy=(77, y), xytext=(72, 50),
                    arrowprops=dict(arrowstyle='->', color=color, lw=2))

    # BMRS Report codes reference
    ax.text(50, 8, 'BMRS Report Codes: ROLSYSDEM (demand) | FUELHH (generation) | FREQ (frequency) | B1770/B1780 (prices)',
            ha='center', va='center', fontsize=8, color='#7f8c8d',
            bbox=dict(boxstyle='round', facecolor='#f8f9fa', edgecolor='#dee2e6', pad=0.5))

    ax.text(50, 3, 'Data Sources: Elexon BMRS © Elexon Limited | Carbon Intensity API © National Grid ESO',
            ha='center', va='center', fontsize=7, color='#adb5bd')

    plt.tight_layout()
    plt.savefig(output_path / 'data_flow_diagram.png', dpi=150, bbox_inches='tight',
                facecolor='white', edgecolor='none')
    plt.close()
...
def create_timeseries_plot(canonical_path: Path, output_path: Path):
    ...
    fig.suptitle('GridBridge UK - GB Grid Data (Real-time Ingestion)', fontsize=14, fontweight='bold')
    ...
    if available_cols:
        ...
        ax1.stackplot(df_plot.index, df_plot.T, labels=[c.replace('_mw', '').upper() for c in available_cols],
                      colors=colors_gen[:len(available_cols)], alpha=0.8)
    ...
    if 'wind_mw' in df.columns:
        ax2.plot(df.index, df['wind_mw']/1000, label='Wind', color='#3498db', linewidth=1.5)
    if 'gas_mw' in df.columns:
        ax2.plot(df.index, df['gas_mw']/1000, label='Gas (CCGT)', color='#e74c3c', linewidth=1.5)
    if 'nuclear_mw' in df.columns:
        ax2.plot(df.index, df['nuclear_mw']/1000, label='Nuclear', color='#9b59b6', linewidth=1.5)
    ...
    ax3 = axes[2]
    if 'carbon_intensity_gco2_kwh' in df.columns and df['carbon_intensity_gco2_kwh'].notna().any():
        ax3.fill_between(df.index, df['carbon_intensity_gco2_kwh'], alpha=0.3, color='#2ecc71')
        ax3.plot(df.index, df['carbon_intensity_gco2_kwh'], color='#27ae60', linewidth=1.5)
        ax3.set_ylabel('Carbon Intensity\n(gCO₂/kWh)', fontsize=10)
        ax3.set_title('Grid Carbon Intensity', fontsize=11, fontweight='bold')
    else:
        ax3.text(0.5, 0.5, 'Carbon intensity data not available', ha='center', va='center',
                 transform=ax3.transAxes, fontsize=12, color='#95a5a6')
    ...
    plt.tight_layout()
    plt.savefig(output_path / 'timeseries_plot.png', dpi=150, bbox_inches='tight',
                facecolor='white', edgecolor='none')
    plt.close()
...
def create_network_diagram(output_path: Path, network_path: Path = None):
    ...
    ax.set_ylim(49, 59)
    ax.set_aspect('equal')
    ax.set_title('GridBridge UK - Simplified GB Transmission Network\n(5-Zone Aggregation)',
                 fontsize=14, fontweight='bold')

    # Zone positions (approximate UK geography)
    zones = {
        'SCOT': {'pos': (-4.0, 56.5), 'label': 'SCOTLAND\n(SPT/SSEN-T)', 'color': '#3498db'},
    ...
    for z1, z2, capacity, label in lines:
        ...
        ax.text(mid[0] + 0.3, mid[1], label, fontsize=7, color='#555',
                ha='left', va='center',
    ...
    for zone, data in zones.items():
        x, y = data['pos']
        circle = Circle((x, y), 0.8, facecolor=data['color'], edgecolor='white',
                        linewidth=3, zorder=2, alpha=0.9)
        ax.add_patch(circle)
        ax.text(x, y, zone, ha='center', va='center', fontsize=11,
                fontweight='bold', color='white', zorder=3)
        ax.text(x, y - 1.3, data['label'], ha='center', va='top', fontsize=8, color='#333')
    ...
    for zone, mix in gen_mix.items():
        ...
        ax.text(x + offset, y, mix, fontsize=7, color='#666',
    ...
    for pos, label, color in interconnectors:
        ...
        ax.text(pos[0], pos[1] - 0.6, label, fontsize=6, ha='center', va='top', color=color)

    # Legend
    legend_elements = [
        Line2D([0], [0], marker='o', color='w', markerfacecolor='#666', markersize=12, label='GSP Zone'),
        Line2D([0], [0], color='black', linewidth=3, label='Transmission (capacity)'),
        Line2D([0], [0], marker='s', color='w', markerfacecolor='#17a2b8', markersize=10, label='Interconnector'),
    ]
    ...
    # Add note
    ax.text(0.5, 0.02, 'Note: Simplified 5-zone aggregation for demonstration. Full model uses ~400 GSPs.',
            transform=ax.transAxes, ha='center', fontsize=8, color='#95a5a6', style='italic')

    plt.tight_layout()
    plt.savefig(output_path / 'network_topology.png', dpi=150, bbox_inches='tight',
                facecolor='white', edgecolor='none')
    plt.close()
...
def create_hidden_capacity_diagram(output_path: Path):
    ...
    ax1 = axes[0]
    ax1.set_title('Hidden Capacity Breakdown\n(Example GSP)', fontsize=12, fontweight='bold')

    categories = ['Physical\nRating', 'Static\nPlanning', 'Queue\nContracted', 'Reported\nHeadroom',
                  'Queue\nAttrition', 'Dynamic\nRating', 'Probabilistic\nN-1', 'TRUE\nHeadroom']
    values = [500, -50, -200, 250, +80, +40, +30, 400]
    colors = ['#3498db', '#e74c3c', '#e74c3c', '#95a5a6', '#27ae60', '#27ae60', '#27ae60', '#2ecc71']
    ...
    for i, (cat, val, color) in enumerate(zip(categories, values, colors)):
        if i == 0:
            ...
            bottom = cumulative if val < 0 else cumulative
            if val < 0:
                bars.append(ax1.bar(i, val, bottom=cumulative, color=color, edgecolor='white', linewidth=2))
                cumulative += val
            else:
                bars.append(ax1.bar(i, val, bottom=cumulative, color=color, edgecolor='white', linewidth=2))
                cumulative += val
        ...
        if i not in [3, 7]:
            ...
            ax1.text(i, label_y + 10, f'{val:+d} MW' if i > 0 else f'{val} MW',
                    ha='center', va='bottom', fontsize=9, fontweight='bold')

    ax1.set_xticks(range(len(categories)))
    ax1.set_xticklabels(categories, fontsize=8, rotation=0)
    ax1.set_ylabel('Capacity (MW)', fontsize=10)
    ax1.axhline(y=250, color='#e74c3c', linestyle='--', linewidth=2, alpha=0.7, label='Reported headroom')
    ax1.axhline(y=400, color='#27ae60', linestyle='--', linewidth=2, alpha=0.7, label='True headroom')
    ax1.legend(loc='upper right', fontsize=9)
    ax1.set_ylim(0, 550)
    ax1.grid(True, alpha=0.3, axis='y')

    # Add annotation
    ax1.annotate('HIDDEN\nCAPACITY\n+150 MW', xy=(5, 320), xytext=(6.5, 450),
    ...
    ax2 = axes[1]
    ax2.set_title('Sources of Hidden Capacity\n(UK Grid Average)', fontsize=12, fontweight='bold')

    sources = ['Queue Attrition\n(40%)', 'Dynamic Line\nRating (25%)', 'Probabilistic\nN-1 (20%)',
               'Conservative\nForecasts (15%)']
    sizes = [40, 25, 20, 15]
    colors = ['#3498db', '#e74c3c', '#9b59b6', '#f39c12']
    explode = (0.05, 0.05, 0.05, 0.05)

    wedges, texts, autotexts = ax2.pie(sizes, explode=explode, labels=sources, colors=colors,
    ...
    ax2.text(0, -1.4, 'Typical UK GSP has 15-40% more usable capacity\nthan reported in static planning studies',
             ha='center', va='top', fontsize=9, color='#666', style='italic')

    plt.tight_layout()
    plt.savefig(output_path / 'hidden_capacity_diagram.png', dpi=150, bbox_inches='tight',
                facecolor='white', edgecolor='none')
    plt.close()
...
def create_flexibility_diagram(output_path: Path):
    ...
    ax.set_ylim(0, 100)
    ax.axis('off')
    ax.set_title('FlexConnect UK - Flexibility Mechanism for Accelerated Connections',
                 fontsize=14, fontweight='bold', pad=20)

    # Connection tiers
    tiers = [
        ('TIER 1: NEAR-FIRM', '<2% curtailment', 'Mission-critical with backup', '#27ae60', 85),
        ('TIER 2: FLEXIBLE', '2-10% curtailment', 'Workload-shiftable compute', '#3498db', 60),
        ('TIER 3: INTERRUPTIBLE', '10-30% curtailment', 'Training runs, batch jobs', '#f39c12', 35),
    ]

    for name, curtail, desc, color, y in tiers:
        box = FancyBboxPatch((5, y-8), 40, 16, boxstyle="round,pad=0.02",
                             facecolor=color, edgecolor='white', linewidth=2, alpha=0.9)
        ax.add_patch(box)
        ax.text(25, y + 3, name, ha='center', va='center', fontsize=11, fontweight='bold', color='white')
        ax.text(25, y - 1, curtail, ha='center', va='center', fontsize=9, color='white')
        ax.text(25, y - 4, desc, ha='center', va='center', fontsize=8, color='white', alpha=0.9)

    # Flexibility assets
    ax.text(72, 92, 'FLEXIBILITY ASSETS', ha='center', va='center', fontsize=12, fontweight='bold', color='#2c3e50')
    ...
    for name, desc, color, y in assets:
        box = FancyBboxPatch((55, y-6), 34, 12, boxstyle="round,pad=0.02",
                             facecolor=color, edgecolor='white', linewidth=2, alpha=0.85)
        ax.add_patch(box)
        ax.text(72, y + 2, name, ha='center', va='center', fontsize=10, fontweight='bold', color='white')
        ax.text(72, y - 2, desc, ha='center', va='center', fontsize=8, color='white', alpha=0.9)

    # Arrows connecting tiers to assets
    ax.annotate('', xy=(55, 78), xytext=(45, 85),
                arrowprops=dict(arrowstyle='->', color='#bdc3c7', lw=1.5))
    ax.annotate('', xy=(55, 58), xytext=(45, 60),
                arrowprops=dict(arrowstyle='->', color='#bdc3c7', lw=1.5))
    ax.annotate('', xy=(55, 38), xytext=(45, 35),
                arrowprops=dict(arrowstyle='->', color='#bdc3c7', lw=1.5))

    # Timeline comparison
    ax.text(50, 18, 'CONNECTION TIMELINE COMPARISON', ha='center', va='center',
            fontsize=11, fontweight='bold', color='#2c3e50')

    # Traditional
    ax.add_patch(FancyBboxPatch((10, 8), 35, 5, boxstyle="round,pad=0.01",
                                facecolor='#e74c3c', edgecolor='white', linewidth=2))
    ax.text(27.5, 10.5, 'Traditional: 5-10 years', ha='center', va='center',
            fontsize=9, fontweight='bold', color='white')

    # FlexConnect
    ax.add_patch(FancyBboxPatch((55, 8), 15, 5, boxstyle="round,pad=0.01",
                                facecolor='#27ae60', edgecolor='white', linewidth=2))
    ax.text(62.5, 10.5, 'FlexConnect: 6-18 months', ha='center', va='center',
            fontsize=9, fontweight='bold', color='white')

    ax.text(85, 10.5, '▶ 70-85%\nfaster', ha='center', va='center',
            fontsize=10, fontweight='bold', color='#27ae60')

    plt.tight_layout()
    plt.savefig(output_path / 'flexibility_diagram.png', dpi=150, bbox_inches='tight',
                facecolor='white', edgecolor='none')
    plt.close()
...
def create_api_coverage_matrix(output_path: Path):
    ...
    data_types = [
        'System Demand', 'Generation by Fuel', 'System Frequency', 'Imbalance Price',
        'Carbon Intensity', 'Weather/Temperature', 'Outage Schedule', 'Queue Data',
        'Capacity Maps', 'Fault Levels'
    ]
    ...
    # Create heatmap
    cmap = plt.cm.RdYlGn
    im = ax.imshow(coverage, cmap=cmap, aspect='auto', vmin=0, vmax=2)
    ...
    ax.set_title('GridBridge UK - Data Source Coverage Matrix', fontsize=14, fontweight='bold', pad=20)
    ...
    plt.tight_layout()
    plt.savefig(output_path / 'api_coverage_matrix.png', dpi=150, bbox_inches='tight',
                facecolor='white', edgecolor='none')
    plt.close()
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Generate GridBridge UK visualizations')
    parser.add_argument('--output', default='out/figures', help='Output directory for figures')
    parser.add_argument('--data', default='out', help='Data directory containing canonical.parquet')
